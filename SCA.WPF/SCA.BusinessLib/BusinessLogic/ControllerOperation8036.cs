                ...
                if (models.Count > 0)
                {
                    ...
                    foreach (var loop in models)
                    {
                        ...
                        foreach (var device in loop.GetDevices<DeviceInfo8036>())
                        {
                            ...
                            excelService.SetCellValue(sheetName, currentRowIndex, 5, device.AlertValue==null?null:device.AlertValue.ToString(), CellStyleType.Data); //报警浓度
                            excelService.SetCellValue(sheetName, currentRowIndex, 6, device.ForcastValue==null?null:device.ForcastValue.ToString(), CellStyleType.Data);  //预警浓度
