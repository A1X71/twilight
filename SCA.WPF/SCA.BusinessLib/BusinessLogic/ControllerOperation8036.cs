    ...
    public class ControllerOperation8036 : ControllerOperationBase, IControllerOperation
    {
        ...
        protected override bool GenerateExcelTemplateStandardSheet(List<string> sheetNames, int currentIndex, int loopSheetAmount, int maxLinkageAmount, ref IExcelService excelService)
        {
            try
            {
                ...
                IControllerConfig config = ControllerConfigManager.GetConfigObject(ControllerType.NT8036);
            ...
            try
            {
                if (models.Count > 0)
                {
                    ...
                    foreach (var loop in models)
                    {
                        ...
                        lstMergeCellRange.Add(mergeCellRange);
                        //写入器件信息         
                        foreach (var device in loop.GetDevices<DeviceInfo8036>())
                        {
                            currentRowIndex++;
                            ...
                            excelService.SetCellValue(sheetName, currentRowIndex, 3, device.LinkageGroup1, CellStyleType.Data); //输出组1
                            excelService.SetCellValue(sheetName, currentRowIndex, 4, device.LinkageGroup2, CellStyleType.Data); //输出组2
                            excelService.SetCellValue(sheetName, currentRowIndex, 5, device.AlertValue, CellStyleType.Data); //报警浓度
                            excelService.SetCellValue(sheetName, currentRowIndex, 6, device.ForcastValue, CellStyleType.Data);  //预警浓度
                            excelService.SetCellValue(sheetName, currentRowIndex, 7, device.DelayValue, CellStyleType.Data);     //延时
                            excelService.SetCellValue(sheetName, currentRowIndex, 8, device.BuildingNo, CellStyleType.Data);     //楼号
                            excelService.SetCellValue(sheetName, currentRowIndex, 9, device.ZoneNo, CellStyleType.Data); //区号
                            excelService.SetCellValue(sheetName, currentRowIndex, 10, device.FloorNo, CellStyleType.Data);//层号
                            excelService.SetCellValue(sheetName, currentRowIndex, 11, device.RoomNo, CellStyleType.Data);//房间号                           
